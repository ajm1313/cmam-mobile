...
export default function WeeklyReportScreen() {
  ...
  const handleSharePDF = async () => {
    ...
    setSharing(true);
    try {
      const periodLabel = `Week ${selWeek || 'All'} - ${['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][selMonth - 1]} ${selYear}`;
      ...
      const tableRows = rows.map(r => `<tr><td>${r.code}</td><td>${r.label}</td><td style="text-align:right;font-weight:700">${r.value}</td></tr>`).join('');

      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
        body { font-family: -apple-system, sans-serif; margin: 0; padding: 24px; color: #1e293b; }
        h1 { font-size: 20px; color: ${typeColor}; margin: 0 0 4px; }
        ...
        th { text-align: left; padding: 8px 12px; background: ${typeColor}; color: #fff; font-weight: 600; }
        ...
        <h1>Health Facility Tally Sheet - ${tab}</h1>
        <h2>Weekly Outpatient Report</h2>
        <div class="meta"><span><b>Period:</b> ${periodLabel}</span><span><b>Location:</b> ${locLabel}</span></div>
        <table><thead><tr><th>Code</th><th>Description</th><th style="text-align:right">Value</th></tr></thead><tbody>${tableRows}</tbody></table>
        <div class="footer">Generated by CMAM Tracker on ${new Date().toLocaleString('en-GB')}</div>
      </body></html>`;

      const { uri: pdfUri } = await Print.printToFileAsync({ html });
      const fileName = `weekly_${tab}_${selMonth}_${selYear}_${Date.now()}.pdf`;
      const finalUri = `${FileSystem.cacheDirectory}${fileName}`;
      await FileSystem.copyAsync({ from: pdfUri, to: finalUri });
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(finalUri, { mimeType: 'application/pdf', dialogTitle: 'Share Weekly Report', UTI: 'com.adobe.pdf' });
      } else {
        Share.share({ message: `Weekly ${tab} Report - ${periodLabel} (${locLabel})`, title: 'CMAM Tracker Report' });
      }
    } catch (e: any) {
